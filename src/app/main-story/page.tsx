...
function getRoleMeta(role = '') {
  ...
  return {
    ...
    date: parts[1] || '',
    category: parts[2]?.toUpperCase() || 'MAIN STORY',
  };
}
...
function mapTestimonialsToPosts(testimonials: PageTestimonial[]): MainStoryPost[] {
  return testimonials
    .filter((item) => item.author || item.quote || item.avatar)
    .map((item, index) => {
      const meta = getRoleMeta(item.role);
      const title = item.author || 'Main Story';

      return {
        id: `${item.author || 'main-story'}-${index}`,
        title,
        author: meta.author,
        date: meta.date,
        image: isValidImageUrl(item.avatar) ? item.avatar!.trim() : fallbackImage,
        excerpt: cleanText(item.quote || 'Read the latest main story from CIO Dialogues.'),
        content: item.quote || 'Read the latest main story from CIO Dialogues.',
...
export default function MainStoryPage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        const response = await fetchWebsitePageBySlug('main-story');
        ...
        // console.error('Failed to load main stories:', error);
    ...
    <main className="news-editorial-page">
      {isLoading ? (
        <section className="news-editorial-status">
          ...
          <p>Fetching the latest main stories.</p>
        </section>
      ) : !leadPost ? (
        <section className="news-editorial-status">
          <h2>No stories found</h2>
          <p>No main stories are available at the moment.</p>
        </section>
      ) : (
        <div className="news-editorial-layout">
              ...
              <ul>
                {sections.map((item) => (
                  <li key={item.label}>
                    <Link
                      href={item.href}
                      className={item.href === '/main-story' ? 'active' : undefined}
                    >
                      {item.label}
                    </Link>
                  </li>
          ...
          <section className="news-editorial-main">
            <article className="news-editorial-lead">
              <Link href={`/main-story/${leadPost.slug}`}>
              ...
              <div className="news-editorial-secondary-grid">
                {secondaryPosts.map((post) => (
                  <article key={post.id} className="news-editorial-secondary-story">
                    ...
                    <Link href={`/main-story/${post.slug}`}>
                      <h2>{post.title}</h2>
                    </Link>
          ...
          <aside className="news-editorial-right-rail">
            <section className="news-editorial-ad">
              ...
              <div className="news-editorial-ad-box">
                ...
                <p>
                  Explore enterprise innovation, artificial intelligence, cloud, security, business
                  transformation, and leadership.
                </p>

                {/* <Link href="/main-story">Explore now</Link> */}
              </div>
            </section>
