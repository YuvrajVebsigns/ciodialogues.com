...
function getRoleMeta(role = '') {
  ...
  return {
    ...
    date: parts[1] || '',
    category: parts[2]?.toUpperCase() || 'BESPOKE',
  };
}
...
function mapTestimonialsToPosts(testimonials: PageTestimonial[]): BespokePost[] {
  return testimonials
    .filter((item) => item.author || item.quote || item.avatar)
    .map((item, index) => {
      const meta = getRoleMeta(item.role || '');
      const title = item.author || 'Bespoke';

      return {
        id: `${item.author || 'bespoke'}-${index}`,
        title,
        author: meta.author,
        date: meta.date,
        image: isValidImageUrl(item.avatar) ? item.avatar!.trim() : fallbackImage,
        excerpt: cleanText(item.quote || 'Read the latest bespoke insight from CIO Dialogues.'),
        content: item.quote || 'Read the latest bespoke insight from CIO Dialogues.',
...
export default function BespokePage() {
  const [posts, setPosts] = useState<BespokePost[]>(staticPosts);
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        ...
        // console.error('Failed to load bespoke stories:', error);
    ...
    <main className="news-editorial-page">
      {isLoading ? (
        <section className="news-editorial-status">
          ...
          <p>Fetching the latest bespoke stories.</p>
        </section>
      ) : !leadPost ? (
        <section className="news-editorial-status">
          <h2>No stories found</h2>
          <p>No bespoke stories are available at the moment.</p>
        </section>
      ) : (
        <div className="news-editorial-layout">
          <aside className="news-editorial-left-rail">
            ...
            <Link href={`/bespoke/${leadPost.slug}`} className="news-editorial-left-feature">
              ...
              <ul>
                {sections.map((item) => (
                  <li key={item.label}>
                    <Link
                      href={item.href}
                      className={item.href === '/bespoke' ? 'active' : undefined}
                    >
                      {item.label}
                    </Link>
                  </li>
          ...
          <section className="news-editorial-main">
            <article className="news-editorial-lead">
              <Link href={`/bespoke/${leadPost.slug}`}>
              ...
              <Link href={`/bespoke/${leadPost.slug}`} className="news-editorial-lead-image">
                <Image
                  src={leadPost.image}
                  alt={leadPost.title}
                  fill
              ...
              <div className="news-editorial-secondary-grid">
                {secondaryPosts.map((post) => (
                  <article key={post.id} className="news-editorial-secondary-story">
                    ...
                    <Link href={`/bespoke/${post.slug}`} className="news-editorial-secondary-image">
                      <Image
                        src={post.image}
                        alt={post.title}
                        fill
                ...
                <div className="news-editorial-latest-grid">
                  {latestPosts.map((post) => (
                    <article key={post.id} className="news-editorial-latest-card">
                      <Link href={`/bespoke/${post.slug}`} className="news-editorial-latest-image">
                        <Image
                          src={post.image}
                          alt={post.title}
                          fill
          ...
          <aside className="news-editorial-right-rail">
            <section className="news-editorial-ad">
              ...
              <div className="news-editorial-ad-box">
                ...
                <h3>
                  Custom technology stories
                  <br />
                  for modern enterprises.
                </h3>

                <p>
                  Explore exclusive content, branded insights, enterprise innovation, technology
                  transformation, and leadership.
                </p>

                {/* <Link href="/bespoke">Explore now</Link> */}
              </div>
            </section>
