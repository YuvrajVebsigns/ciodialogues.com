...
function getBlogImage(blog: WebsiteBlogItem) {
  ...
  return isValidImageUrl(candidate) ? candidate : FALLBACK_BLOG_IMAGE;
}
...
function AnimatedBlogCard({ blog, index, variant = 'animate-fade-in' }: AnimatedBlogCardProps) {
  ...
  const [imageSrc, setImageSrc] = useState<string>(() => getBlogImage(blog));
  ...
  }, [blog.id, blog.featureImage, blog.seo?.ogImage, likesCount]);
    ...
    <article ref={ref} className="blogpage-card" style={{ transitionDelay: `${index * 60}ms` }}>
      <div className="blogpage-image-wrap">
        <Image
          ...
          unoptimized
          onError={() => setImageSrc(FALLBACK_BLOG_IMAGE)}
        />
      </div>
