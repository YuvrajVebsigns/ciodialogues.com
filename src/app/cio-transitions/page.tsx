...
const staticPosts: CIOTransitionPost[] = [];
...
function getRoleMeta(role = '') {
  ...
  return {
    ...
    date: parts[1] || '',
    category: parts[2]?.toUpperCase() || 'CIO TRANSITIONS',
  };
}
...
function mapTestimonialsToPosts(testimonials: PageTestimonial[]): CIOTransitionPost[] {
  return testimonials
    .filter((item) => item.author || item.quote || item.avatar)
    .map((item, index) => {
      const meta = getRoleMeta(item.role || '');
      const title = item.author || 'CIO Transition';

      return {
        id: `${item.author || 'cio-transition'}-${index}`,
        title,
        author: meta.author,
        date: meta.date,
        image: isValidImageUrl(item.avatar) ? item.avatar!.trim() : fallbackImage,
        excerpt: cleanText(
          item.quote || 'Read the latest CIO transition update from CIO Dialogues.',
        ),
        content: item.quote || 'Read the latest CIO transition update from CIO Dialogues.',
...
export default function CIOTransitionsPage() {
  const [posts, setPosts] = useState<CIOTransitionPost[]>(staticPosts);
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        ...
        // console.error('Failed to load CIO transition stories:', error);
    ...
    <main className="news-editorial-page">
      {isLoading ? (
        <section className="news-editorial-status">
          ...
          <p>Fetching the latest CIO transitions.</p>
        </section>
      ) : !leadPost ? (
        <section className="news-editorial-status">
          <h2>No stories found</h2>
          <p>No CIO transition stories are available at the moment.</p>
        </section>
      ) : (
        <div className="news-editorial-layout">
          <aside className="news-editorial-left-rail">
            ...
            <Link
              href={`/cio-transitions/${leadPost.slug}`}
              ...
              <ul>
                {sections.map((item) => (
                  <li key={item.label}>
                    <Link
                      href={item.href}
                      className={item.href === '/cio-transitions' ? 'active' : undefined}
                    >
                      {item.label}
                    </Link>
                  </li>
          ...
          <section className="news-editorial-main">
            <article className="news-editorial-lead">
              <Link href={`/cio-transitions/${leadPost.slug}`}>
              ...
              <div className="news-editorial-secondary-grid">
                {secondaryPosts.map((post) => (
                  <article key={post.id} className="news-editorial-secondary-story">
                    ...
                    <Link href={`/cio-transitions/${post.slug}`}>
                      <h2>{post.title}</h2>
                    </Link>
                    ...
                    <Link
                      href={`/cio-transitions/${post.slug}`}
            ...
            <section className="news-editorial-ad">
              ...
              <div className="news-editorial-ad-box">
                ...
                <h3>
                  CIO appointments
                  <br />
                  shaping enterprise technology.
                </h3>

                <p>
                  Follow CIO appointments, executive movements, leadership changes, and strategic
                  technology transitions across industries.
                </p>

                {/* <Link href="/cio-transitions">Explore now</Link> */}
              </div>
            </section>
